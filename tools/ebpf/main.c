...
// to a raw AF_PACKET socket. It periodically writes JSON objects containing
// SSH (22) & HTTPS (443) packet counts, plus an HTTP snippet from port 8080,
// to /tmp/ebpf_kovid.json.
//
// Usage example:
//   1) Place socket_filter_bpf.o in /usr/bin/
//   2) Launch a local server:  python3 -m http.server 8080 --bind 127.0.0.1
//   3) Generate traffic:       wget http://127.0.0.1:8080/
//   4) Run as root:            sudo /usr/bin/ebpf-kovid
//   5) Tail /tmp/ebpf_kovid.json to see new JSON objects.
...
int main(int argc, char **argv)
{
    ...
    int prog_fd = -1, map_fd = -1, snippet_map_fd = -1;
    int sock_fd = -1, err;
    ...
    // Find the eBPF program by section
    ...
    // Create a raw packet socket and attach eBPF
    ...
    // Open /tmp/ebpf_kovid.json for appending
    FILE *fp = fopen("/tmp/ebpf_kovid.json", "a");
    if (!fp) {
        fprintf(stderr, "ERROR: Could not open /tmp/ebpf_kovid.json for writing.\n");
        goto cleanup;
    }

    fprintf(fp, "// eBPF KoviD JSON Log\n");
    fflush(fp);

    // 6) Periodically read counters & snippet, log to JSON
    ...
    while (1) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (bpf_map_lookup_elem(map_fd, &ports[i], &value) == 0) {
                ...
                fprintf(fp, "    { \"port\": %u, \"packets\": %llu }%s\n",
                        ports[i], value, (i == 0 ? "," : ""));
            } else {
                fprintf(fp, "    { \"port\": %u, \"packets\": 0 }%s\n",
                        ports[i], (i == 0 ? "," : ""));
            }
        }
        ...
        // (b) Optional HTTP snippet
        {
            ...
            if (bpf_map_lookup_elem(snippet_map_fd, &key, &snippet) == 0) {
                ...
                // If the snippet lookup fails, also set null
                fprintf(fp, "  \"http_snippet\": null,\n");
            }
        }
